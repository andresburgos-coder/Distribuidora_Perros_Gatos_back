...
DB_NAME="distribuidora_db" # AÃƒÆ’Ã‚Â±adimos el nombre de la base de datos para futuras referencias

# Esperar a que SQL Server estÃƒÆ’Ã‚Â© listo
...
do
    ...
    if [ "$ATTEMPT" -ge "$MAX_RETRIES" ]; then
        echo "--- ERROR FATAL DE CONEXIÃƒÆ’Ã¢â‚¬Å“N ---"
        cat connection_error.log || true
        echo "El servidor SQL no respondiÃƒÆ’Ã‚Â³ despuÃƒÆ’Ã‚Â©s de $MAX_RETRIES intentos. La inicializaciÃƒÆ’Ã‚Â³n ha fallado por timeout."
        echo "VERIFIQUE: 1. Los lÃƒÆ’Ã‚Â­mites de memoria/CPU en docker-compose. 2. Los logs del contenedor 'sqlserver' para ver si estÃƒÆ’Ã‚Â¡ en recuperaciÃƒÆ’Ã‚Â³n."
        exit 1
    fi
